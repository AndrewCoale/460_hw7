...
(define-type Exp
  ...
  (sendE [obj-expr : Exp]
         [method-name : Symbol]
         [arg-expr : Exp])
  (selectE [some-numb : Exp] ; copied in class, part of #2
           [some-obj : Exp]) ;
  (ssendE [obj-expr : Exp]
          [class-name : Symbol]
          [method-name : Symbol]
    ...
    (local [(define (recur expr)
      ...
      (type-case Exp a
        ...
        [(newE class-name field-exprs)
         (local [(define c (if (symbol=? class-name 'Object) ; problem 1 solution
                               (classC empty empty)          ; change to (classC 'Object empty empty) later
           ...
           (type-case Value obj
             [(objV class-name field-vals)
              (call-method class-name method-name classes
                           obj arg-val)]
             [else (error 'interp "not an object")]))]
        [(selectE arg-expr obj-expr)                                                           ; copied from class for #2
         (local [(define arg-val (recur arg-expr))                                             ;
                 (define obj (recur obj-expr))]                                                ;
           (type-case Value arg-val                                                            ;
             [(numV n)                                                                         ;
              (type-case Value obj                                                             ;
                [(objV class-name field-vals)                                                  ;
                 (call-method class-name (if (equal? arg-val (numV 0)) 'zero 'nonzero) classes ;
                              obj arg-val)]                                                    ;
                [else (error 'interp "not an object")])]                                       ;
...
(define-type ExpI
  ...
  (sendI [obj-expr : ExpI]
         [method-name : Symbol]
         [arg-expr : ExpI])
  (selectI [num : ExpI]  ; added as part of #2
           [obj : ExpI]) ;
    ...
    (type-case ExpI a
      ...
      [(sendI expr method-name arg-expr)
       (sendE (recur expr)
              method-name
              (recur arg-expr))]
      [(selectI num-val arg-expr)  ; added as part of #2
       (selectE (recur num-val)    ;
                (recur arg-expr))] ;
...
(define (parse [s : S-Exp]) : ExpI
  (cond
   ...
   [(s-exp-match? `{select ANY ANY} s)        ; added as part of #2
...
(module+ test
  ...
  (test (interp-prog (list `{class Snowball extends Object
        ...
        `2)
  (test/exn (interp (selectE (argE) (numE 1)) 
                empty (objV 'Object empty) (numV 0)) "not an object")
  (test/exn (interp-prog (list `{class Snowball extends Object
                              {size}
                              [zero {arg} this]
                              [nonzero {arg}
                                       {new Snowball {+ 1 {get this size}}}]})
                     `{get {select this {new Snowball 1}} size}) "not a number"))