...
(define-type Exp
  ...
  (sendE [obj-expr : Exp]
         [method-name : Symbol]
         [arg-expr : Exp])
  (selectE [some-numb : Exp] ; copied in class
           [some-obj : Exp]) ;
  (ssendE [obj-expr : Exp]
          [class-name : Symbol]
          [method-name : Symbol]
...
(define-type Class ; definition will need to be modified
...
(define interp : (Exp (Listof (Symbol * Class)) Value Value -> Value)
  (lambda (a classes this-val arg-val)
    (local [(define (recur expr)
              (interp expr classes this-val arg-val))] ; think of this as the parameter pass
      (type-case Exp a
        [(numE n) (numV n)]
